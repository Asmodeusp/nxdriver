...
public class RecognizeService {
    ...
    public static void recCustom(Context ctx, String filePath, String classifierId,
        ...
        OcrRequestParams param = new OcrRequestParams();
        param.putParam("templateSign", templateSign);//模板ID
        //param.putParam("classifierId", classifierId);//类别ID
        param.setImageFile(new File(filePath));
        OCR.getInstance(ctx).recognizeCustom(param, new OnResultListener<OcrResponseResult>() {
            @Override
            public void onResult(OcrResponseResult result) {
                Log.e("msg","识别成功"+result.getJsonRes());
                listener.onResult(result.getLogId()+"");
            }

            @Override
            public void onError(OCRError error) {
                listener.onResult(error.getLogId()+"错误码："+error.getErrorCode()+"错误"+error.getMessage());
            }
        });
    }
