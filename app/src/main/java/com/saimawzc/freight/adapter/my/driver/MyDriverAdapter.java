...
public class MyDriverAdapter extends BaseAdapter {
    ...
    public MyDriverAdapter(List<MyDriverDto> mDatas, Context mContext) {
        this.mDatas = mDatas;
        this.mContext = mContext;
        mInflater = LayoutInflater.from(mContext);
        activity=(BaseActivity) mContext;
    }
    public MyDriverAdapter(List<MyDriverDto> mDatas, Context mContext, int type) {
        this.mDatas = mDatas;
        this.mContext = mContext;
        mInflater = LayoutInflater.from(mContext);
        activity=(BaseActivity) mContext;
    ...
    public void setData(List<MyDriverDto> mDatas ) {
        this.mDatas = mDatas;
        notifyDataSetChanged();
    }

    public List<MyDriverDto> getData() {
        return mDatas;
    }
    public void addMoreData(List<MyDriverDto> newDatas) {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof ViewHolder){
            final MyDriverDto myDriverDto=mDatas.get(position);
             ((ViewHolder) holder).imgUnbind.setVisibility(View.VISIBLE);

            if(myDriverDto.getDriverType()==2){
                ((ViewHolder) holder).tvName.setText(myDriverDto.getRoleName()+"（船员）");
            }else {
                ((ViewHolder) holder).tvName.setText(myDriverDto.getRoleName()+"（司机）");
            }
    ...
    private void unbindCarRelation(String carId){
        ...
        mineApi.unbindCarriveRelation(body).enqueue(new CallBack<EmptyDto>() {
            ...
            public void success(EmptyDto response) {
                ...
                EventBus.getDefault().post(Constants.reshMyDriver);
            }
