...
public class SearchCarAdapter extends BaseAdapter {
    ...
    public SearchCarAdapter(List<SearchCarDto> mDatas, Context mContext) {
        this.mDatas = mDatas;
        this.mContext = mContext;
        mInflater = LayoutInflater.from(mContext);
        activity=(BaseActivity) mContext;
    }
    public SearchCarAdapter(List<SearchCarDto> mDatas, Context mContext,int type) {
        this.mDatas = mDatas;
        this.mContext = mContext;
        mInflater = LayoutInflater.from(mContext);
        activity=(BaseActivity) mContext;
    ...
    public void setData(List<SearchCarDto> mDatas ) {
        this.mDatas = mDatas;
    ...
    public void addMoreData(List<SearchCarDto> newDatas) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            View view = mInflater.inflate(R.layout.item_searchcar, parent,
                    false);
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof ViewHolder){
            final SearchCarDto searchCarDto=mDatas.get(position);
            Log.e("getIfDisable",searchCarDto.getIfDisable()+"")  ;
            Log.e("getIsBlackList",searchCarDto.getIsBlackList()+"")  ;
            if (searchCarDto.getIsBlackList()!=null&&searchCarDto.getIfDisable()!=null) {
                if (searchCarDto.getIsBlackList()==1||searchCarDto.getIfDisable()==2) {
                    ((ViewHolder) holder).tvCardNum.setTextColor(mContext.getResources().getColor(R.color.gray888));
                    ((ViewHolder) holder).tvCarShapeAp.setTextColor(mContext.getResources().getColor(R.color.gray888));
                    ((ViewHolder) holder).tvweightAp.setTextColor(mContext.getResources().getColor(R.color.gray888));
                    ((ViewHolder) holder).tvuserAp.setTextColor(mContext.getResources().getColor(R.color.gray888));
                }
            }else {
                if (searchCarDto.getIfDisable()!=null) {
            ...
            if(searchCarDto!=null){
                ((ViewHolder) holder).tvCardNum.setText(searchCarDto.getCarNo());
                ((ViewHolder) holder).tvCarShape.setText(searchCarDto.getCarTypeName());
                ((ViewHolder) holder).tvWeight.setText(searchCarDto.getCarLoad());
                ((ViewHolder) holder).tvUser.setText(searchCarDto.getCarName());
                ImageLoadUtil.displayImage(mContext,searchCarDto.getVehicleSurvey(),((ViewHolder) holder).imageView);
            }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        @BindView(R.id.tvCarShapeAp)TextView tvCarShapeAp;
        @BindView(R.id.tvweightAp)TextView tvweightAp;
        @BindView(R.id.tvuserAp)TextView tvuserAp;
