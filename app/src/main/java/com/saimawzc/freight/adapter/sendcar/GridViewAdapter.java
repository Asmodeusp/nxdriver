...
public class GridViewAdapter extends android.widget.BaseAdapter {
    ...
    public int getCount() {
        ...
        if (count > 20) {//MainConstant.MAX_SELECT_PIC_NUM最多上传的图片数
            return mList.size();
        } else {
            return count;
        }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (position < mList.size()) {
            ...
            Glide.with(mContext.getApplicationContext()).load(picUrl).into(iv);
        } else {
            iv.setImageResource(R.drawable.ico_addpic);//最后一个显示加号图片
        }
        return convertView;
    }
}  
